        ...
        private void btnChange_Click(object sender, EventArgs e) //Кнопка Изменить
        {
            ...
            if (listBox1.SelectedIndex != -1)
            {
                ...
                else
                {
                    ...
                    ClassStudents updated = new ClassStudents(surname, name, current.Birthday, current.PhoneNumber, current.Adress, current.PhotoUrl);
                    ClassStudents.RemoveStudent(listBox1.SelectedIndex);
                    List<ClassStudents> all = new List<ClassStudents>();
                    for (int i = 0; i < ClassStudents.CountStudents(); i++)
                    {
                        all.Add(ClassStudents.GetStudents(i));
                    }
                    ...
                    for (int i = 0; i < all.Count; i++)
                    {
                        if (i == listBox1.SelectedIndex)
                        {
                            ClassStudents.AddStudent(updated);
                        }
                        ClassStudents.AddStudent(all[i]);
                    }
                    UpdateListBox();
                }
            }
        ...
        private void btnInsert_Click(object sender, EventArgs e) //Кнопка Вставить
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    ClassStudents newStudent = new ClassStudents(surname, name, DateTime.Now, "", "", "");
                    int insertIndex = listBox1.SelectedIndex;
